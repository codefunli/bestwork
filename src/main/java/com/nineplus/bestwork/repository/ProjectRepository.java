...
public interface ProjectRepository extends JpaRepository<ProjectEntity, String> {
	...
	@Query(value = "select ast.company_id as companyId, tc.company_name as companyName, tus.user_name as name, ast.user_id as userId, ast.can_view as canView , ast.can_edit as canEdit from T_COMPANY tc join ASSIGN_TASK ast ON tc.id = ast.company_id JOIN PROJECT pr ON ast.project_id = pr.id JOIN T_SYS_APP_USER tus ON tus.id = ast.user_id WHERE ast.project_id = :projectId", nativeQuery = true)
